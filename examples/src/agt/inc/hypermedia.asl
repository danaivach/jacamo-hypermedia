...
+!load_environment(EnvName, EnvUrl) : true <-
  ...
  makeArtifact(EnvName, "org.hyperagents.jacamo.artifacts.yggdrasil.ContainerArtifact", [EnvUrl, "workspace"], ArtId);
  focus(ArtId);
...
+workspace(WorkspaceIRI, WorkspaceName) : true <-
  ...
  makeArtifact(WorkspaceName, "org.hyperagents.jacamo.artifacts.yggdrasil.ContainerArtifact", [WorkspaceIRI, "artifact"], WkspArtId)[wid(WorkspaceId)];
  focus(WkspArtId);
